...
// d.m.v. de functie veld_ikzoek(). Resultaat is afhankelijk van
// het aantal aanroepen en je groepsnummer, zie speelveld.h.
const VELD_GPS veld_zoek[] = {
	{ 52.079216f, 5.170761f },  // ZW (ver weg van het speelveld)
	{ 52.080728f, 5.173576f },  // centraal (open terrein)
	{ 52.081118f, 5.174240f },  // NO (open terrein)
...
// Is deze positie op het speelveld? (= binnen de grenzen van de veelhoek?)
...
{
	// Array-lengte van de veelhoek die de rand van het speelveld beschrijft.
	...
	unsigned int i, j;  // naastliggende punten, dus i<->j is altijd een lijnstuk
	...
	if (firstrun)
	{
		...
		for (i = 0; i < randpunten; ++i)      // bepaal min/max van alle punten
		{
			...
			if (f < boundingbox[0].lat)       // kleiner?
				boundingbox[0].lat = f;       // minimale lat
			else if (f > boundingbox[1].lat)  // groter?
				boundingbox[1].lat = f;       // maximale lat
			f = veld_rand[i].lon;             // huidige lon
			if (f < boundingbox[0].lon)       // kleiner?
				boundingbox[0].lon = f;       // minimale lon
			else if (f > boundingbox[1].lon)  // groter?
				boundingbox[1].lon = f;       // maximale lon
		}
	}

	// Niet op het veld als het buiten de buitenste randen is
	if (pos->lat < boundingbox[0].lat ||
		pos->lat > boundingbox[1].lat ||
		pos->lon < boundingbox[0].lon ||
		pos->lon > boundingbox[1].lon)
		return 0;

	j = randpunten - 1;  // naastliggend aan i = 0
	for (i = 0; i < randpunten; ++i)
	{
		...
		j = i;  // volgende lijnstuk, weer naastliggende waarde van j
	}
...
VELD_GPS veld_ikzoek(void)
{
	static const unsigned int zoekpunten = (sizeof veld_zoek) / (sizeof veld_zoek[0]);
	static unsigned int n = VELD_PROJECTGROEP - 1;  // begin met 0 want meteen ++

	++n;              // volgende punt
	n %= zoekpunten;  // blijf binnen de array
